...
export class BloqueoDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data.bloqueo) {
      this.form.patchValue({
        empleadoId: this.data.bloqueo.empleadoId,
  ...
  onSubmit(): void {
    ...
    const empleado = this.data.empleados.find((e) => e.id === Number(formValue.empleadoId));

    const resultado: BloqueoAgenda = {
      id: this.data.bloqueo?.id ?? 0,
      empleadoId: Number(formValue.empleadoId),
      empleadoNombre: empleado ? [empleado.nombre, empleado.apellido].filter(Boolean).join(' ') : '',
